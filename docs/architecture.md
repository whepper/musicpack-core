...
| **Container sources (client + server)** | `crates/musicpack-engine/tests/mpak_range_source.rs` (range-backed member ≡ whole member, PCM-identical; block-aligned host, cache reuse, error paths) and `crates/musicpack-server/tests/mpak_ingest.rs` (container discovery → `MpakBackend` → MANF → index → canonical `mpak:<container>#<member>` → member bytes; invalid container, container verifier, directory regression) + `crates/musicpack-server/tests/mpak_media_serving.rs` (HTTP byte serving of a member: full/bounded/open-ended/suffix ranges, member-relative `Content-Range`, end-of-member and out-of-range boundaries, two members, hostile member paths, corrupt container, 1.5 MB member), all over the committed `fixtures/reference/reference-small.mpak`. The shared key form's vectors live in `src/player/source_url.rs` | established; server ingestion is a deliberate **superset** of the C scanner (which only walked `.mpack` dirs), pinned in `crates/musicpack-server/tests/discovery.rs` |
